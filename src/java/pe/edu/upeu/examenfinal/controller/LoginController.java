...
public class LoginController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<LoginDTO> login = udao.login(username, pass);
        if (!login.isEmpty()) {
            sesion.setAttribute("nombre", login.get(0).getNombre());
            sesion.setAttribute("apellidos", login.get(0).getApellidos());
            sesion.setAttribute("username", login.get(0).getUsername());
            sesion.setAttribute("rol", login.get(0).getRol());
            request.setAttribute("user", login.get(0).getUsername());
